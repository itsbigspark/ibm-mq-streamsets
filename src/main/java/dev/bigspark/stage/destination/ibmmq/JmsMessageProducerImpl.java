...
import dev.bigspark.lib.ibmmq.DefaultErrorRecordHandler;
import dev.bigspark.lib.ibmmq.ErrorRecordHandler;
...
public class JmsMessageProducerImpl implements JmsMessageProducer {
    ...
    public List<Stage.ConfigIssue> init(Target.Context context) {
        ...
        if (issues.isEmpty() && StringUtils.isNotBlank(jmsTargetConfig.replyToQueue)) {
            try {
                replyToDestination = session.createQueue(jmsTargetConfig.replyToQueue);
            } catch (JMSException ex) {
                issues.add(context.createConfigIssue(JmsGroups.JMS.name(), CONN_FACTORY_CONFIG_NAME, JmsErrors.JMS_15,
                        jmsTargetConfig.replyToQueue, ex.toString()));
                LOG.info(Utils.format(JmsErrors.JMS_15.getMessage(), jmsTargetConfig.replyToQueue, ex.toString()), ex);
            }
        }
        if (issues.isEmpty()) {
                        ...
                        public MessageProducer load(String key) throws Exception {
                            switch (jmsTargetConfig.destinationType) {
                                case UNKNOWN:
                                    ...
                                    break;
                                case QUEUE:
                                    destination = session.createQueue(key + (jmsTargetConfig.removeRFH2Header ? "?targetClient=1" : ""));
                                    break;
                                case TOPIC:
    ...
    private void handleDelivery(Record record, byte[] payload) throws StageException {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(jmsTargetConfig.replyToQueue)) {
